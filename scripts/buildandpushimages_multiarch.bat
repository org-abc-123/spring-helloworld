...
echo "building and pushing image demo-buildstage"
pushd source\output\src
docker buildx build --platform ${PLATFORMS} -f Dockerfile.buildstage --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/demo-buildstage .
popd

echo "building and pushing image demo"
pushd source\output\src
docker buildx build --platform ${PLATFORMS} -f Dockerfile --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/demo .
popd
